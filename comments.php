					...
					<h3>
						...
						if ('1' === $loveus_comment_count) {
							...
							printf(// WPCS: XSS OK.
									...
									esc_html(_nx('%1$s Comment', '%1$s Comments ', $loveus_comment_count, 'comments title', 'loveus'), 'loveus'), number_format_i18n($loveus_comment_count)
							);
						}
						?>
					</h3>
...
<div class="comment-form-area default-form <?php echo esc_attr($is_no_post_thumb); ?>">
		...
		<?php
			...
			$formargs = array(
				...
				'must_log_in' => '<div>' .
				sprintf(
						wp_kses(__('You must be <a href="%s">logged in</a> to post a comment.', 'loveus'), array('a' => array('href' => array()))), wp_login_url(apply_filters('the_permalink', get_permalink()))
				) . '</div>',
				'logged_in_as' => '<div class="logged-in-as">' .
				sprintf(
						wp_kses(__('Logged in as <a href="%1$s">%2$s</a>. <a href="%3$s" title="%4$s">Log out?</a>', 'loveus'), array('a' => array('href' => array()))), esc_url(admin_url('profile.php')), $loveus_user_identity, wp_logout_url(apply_filters('the_permalink', get_permalink())), esc_attr__('Log out of this account', 'loveus')
				) . '</div>',
